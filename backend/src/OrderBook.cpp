...
namespace myex{
    ...
    std::optional<Order>OrderBook::peekBid() const{
        ...
        else return bids_.begin()->second.front();
    }
    std::optional<Order>OrderBook::peekAsk() const{
        if(asks_.empty()) return std::nullopt;
        else return asks_.begin()->second.front(); 
    }

    size_t OrderBook::bidOrderCount() const {
        return bid_count_;
    }
    size_t OrderBook::askOrderCount() const {
        return ask_count_;
    }
            ...
            if(map_it != bids_.end()){
                ...
                for(auto dq_it = dq.begin(); dq_it != dq.end(); ++dq_it){
                    if(dq_it->id == order_id){
                        dq.erase(dq_it);
                        bid_count_--;
                        if(dq.empty()) bids_.erase(map_it);         //if deque now empty → erase price level from bids_
                        return true;
                    } 
                }
            }
            ...
            if(map_it != asks_.end()){
                ...
                for(auto dq_it = dq.begin(); dq_it != dq.end(); ++dq_it){
                    if(dq_it->id == order_id){
                        dq.erase(dq_it);
                        ask_count_--;
                        if(dq.empty()) asks_.erase(map_it);
                        return true;
                    }
                }
            }
